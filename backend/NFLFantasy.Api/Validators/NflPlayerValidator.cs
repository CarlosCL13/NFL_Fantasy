...
namespace NFLFantasy.Api.Validators
{
    public class NflPlayerValidator
    {
        ...
        public (bool IsValid, string? Error) ValidateCreate(NflPlayerCreateDto dto, NflPlayerRepository repository)
        {
            // Validar campos requeridos (ya lo hace el modelo, pero por si acaso)

            if (string.IsNullOrWhiteSpace(dto.Name) || dto.PositionId <= 0 || dto.Image == null)
